...
function rangliste_data() {
	global $rangliste_data;
	...
	foreach ($rankings as $ranking) {
		...
		$result = mysql_query("
			...
			ON DUPLICATE KEY UPDATE
				`total` = `total` + $punkte,
				`sekundaer` = `sekundaer` + $punkte;
		");
//		if (!$result) {
//			$aseco->console('[konte.rangliste.php] Error on inserting data into `rangliste`: '. mysql_error());
//		}
	}
